...
namespace ArchCore.Networking.WebSocket
{
	public abstract class WebSocketClient
	{
		...
		private readonly Dictionary<string, ServerPushDelegate> serverPushHandlers =
			new Dictionary<string, ServerPushDelegate>();

		private readonly Dictionary<string, ServerErrorDelegate> serverErrorHandlers =
			new Dictionary<string, ServerErrorDelegate>();
		...
		public void RegisterServerPushHandler<T>(string pushCode, ServerPushDelegate handler,
			ServerErrorDelegate errorHandler = null)
		{
		...
		private void ProcessMessage(WebSocketMessageInfo info)
		{
			if (info.Exception == null)
			{
				if (!string.IsNullOrEmpty(info.ServerPushCode) &&
				    serverPushHandlers.TryGetValue(info.ServerPushCode, out var handler))
				{
		...
		private void HandleError(BestHTTP.WebSocket.WebSocket websocket, Exception ex)
		{
			ClearSocket();
			promise.DisconnectComplete(ex);
		}
